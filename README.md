...
系统采用深色科技风 UI，支持桌面端与移动端。以下为各主要页面截图：

### 登录 / 注册

![登录页](docs/screenshots/login.png)

*登录页支持用户名、密码与 SVG 验证码；底部可跳转注册。*

### 我的日志（普通用户）

![我的日志](docs/screenshots/my-logs.png)

*按月浏览与填报工时日志，含项目工时统计、日期标签与当日日志详情。*

### 个人信息

![个人信息](docs/screenshots/profile.png)

*用户可修改显示姓名与密码（密码长度需超过 6 位）。*

### 日志查询（管理员）

![日志查询](docs/screenshots/admin-logs.png)

*多条件筛选、项目工时统计、人员投入柱形图、日志明细与考勤合并展示，支持导出 Excel 及导入考勤/历史日志。*

### 项目管理（管理员）

![项目管理](docs/screenshots/admin-projects.png)

*项目 CRUD、别名维护、负责人指定与启用/禁用；顶部展示项目与工时汇总统计。*

### 人员管理（管理员）

![人员管理](docs/screenshots/admin-users.png)

*审核注册用户、重置密码，按角色与审核状态筛选，顶部展示人员统计概览。*
...
- [x] 选择项目时显示人员投入柱形图
- [x] 日志列表合并考勤列
- [x] 导出 Excel（含审核方式：人工审核 / 自动审核）
- [x] **导入考勤**：上传月度考勤汇总 Excel
- [x] **导入日志**：上传历史填报 Excel 并指定年份
- [x] **导入原系统日志**：上传单项目原系统导出 Excel，自动匹配/创建人员并导入
- [x] **开放日志**：指定日期开放 12 小时，期间用户可补填该日日志
- [x] **删除日志**：删除指定日志后，单独为该人员开放该日 12 小时补填权限
...
| 工时精度 | 保留 2 位小数，必须 &gt; 0 |
| 项目选择 | 仅可选择「启用」状态的项目 |
| 管理员开放 | 管理员开放某日期后，**12 小时内**全站用户可为该日新增日志（绕过 17:00 与次日 12:00 限制） |
| 单人补填开放 | 管理员删除某人员某日日志后，**仅该人员**在 **12 小时内**可重新填报该日日志 |
...
| PUT | `/api/admin/users/<id>/display_name` | 修改用户姓名 |
...
| POST | `/api/admin/logs/import-legacy` | 导入原系统日志 Excel |
| GET | `/api/admin/logs/open` | 查询当前进行中的开放记录 |
| POST | `/api/admin/logs/open` | 开放指定日期日志（12 小时有效） |
| DELETE | `/api/admin/logs/<id>` | 删除日志并为该人员单独开放该日 12 小时 |
...
| `log_open_periods` | 管理员全站开放的日志填报日期及有效期 |
| `user_log_open_periods` | 管理员为指定人员开放的日志填报日期及有效期 |
...
### 13.4 原系统日志 Excel（原系统日志导入）

- 格式：`.xlsx` / `.xls`
- 必须包含列：**项目名称**、**姓名**、**日志日期**、**工时**、**日志内容**
- 文档中所有行的 **项目名称必须相同**，且能在本系统项目管理中通过名称或别名匹配
- **姓名** 按显示名称匹配系统用户；不存在的姓名将自动创建账号（状态为已拒绝/禁用，不可登录）
- 按日志日期、工时、工作内容写入数据库；同一用户 + 项目 + 日期重复则更新
- 今日以前的记录默认 `approved`；今日及以后为 `pending`；未来日期跳过

### 13.5 日志查询导出 Excel

列：姓名、用户名、日期、星期、项目、工时（小时）、工作内容、考勤、审核方式、审核人、创建时间

审核方式规则：
- **人工审核**：项目负责人手动通过或驳回
- **自动审核**：超过日志日次日 12:00 未处理而自动通过

审核人：对应该日志所属项目的项目负责人姓名；项目未设置负责人时留空。
